...
### `muteConferenceLocalPlayback(accountId: String, confId: String, mute: Boolean): Boolean`
Локальное выключение воспроизведения конференции только для хоста.
- `accountId`: ID учетной записи.
- `confId`: ID конференции или ID вызова-участника этой конференции.
- `mute`: `true` — хост перестаёт слышать конференцию в динамиках; `false` — вернуть локальное
  воспроизведение.

Возвращает `true`, если конференция найдена и состояние применено.
Логика: вызывает `conf->muteLocalPlayback`. Не влияет на слышимость между удалёнными участниками и
не меняет mute самих участников конференции.
