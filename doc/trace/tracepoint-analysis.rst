===================================
Analyzing sip_core's static tracepoints
===================================

:Author: Olivier Dion <olivier.dion@savoirfairelinux.com>

This documentation explains how to create a tracing session, enabling sip_core's
...
For now, let's create our session like so ``lttng create sip_core``.  If you then do
``lttng status`` you should have something like::

  Recording session sip_core: [inactive]
    Trace output: /home/old/lttng-traces/sip_core-20211213-112959
...
But your ``sip_core`` session is still there.  Try ``lttng list`` to confirm::
...
We can now do ``lttng set-session sip_core && lttng destroy temp`` to set the
current session to ``sip_core`` and destroy the ``temp`` session.  You should now be
using the ``sip_core`` session for the rest of this document.
...
add all events from the provider ``sip_core`` to our session with ``lttng enable-event --userspace 'sip_core:*'``
::
   ust event sip_core:* created in channel channel0
...
Once this is done, try ``lttng status`` to see what happen::
  ...
  - channel0: [enabled]
    ...
    Recording event rules:
      sip_core:* (type: tracepoint) [enabled]
...
``sip_core:*`` for the ``channel0``.  Our rule is globing all events produced by the
``sip_core`` provider.  Other informations are documented in the LTTng'
documentation.

NOTE!  Do not confuse the ``sip_core`` session with the ``sip_core`` provider.  Even
though they have the same name in this document, you could have a session named
``foo`` instead of ``sip_core`` and everything in this document should work the same.
...
You can now start the session with ``lttng start``::
  ...
  Tracing started for session sip_core

and run sip_core ``./bin/sip_cored``.

Let it run a few seconds and stop the sip_core's daemon.  Then you can run ``lttng
destroy`` (or ``lttng stop`` if you do not want to destroy) to stop and destroy
your session::

  Destroying session sip_core...
  Session sip_core destroyed
...
is your trace folder.  You should now have something like this::
  ...
  [12:05:05.899862574] (+?.?????????) laura sip_core:scheduled_executor_task_begin: { cpu_id = 13 }, { perf_thread_cycles = 49068, perf_thread_cpu_migrations = 0 }, { executor = "natpmp", source_filename = "upnp/protocol/natpmp/nat_pmp.cpp", source_line = 233, cookie = 0 }
  [12:05:05.916075225] (+0.016212651) laura sip_core:scheduled_executor_task_end: { cpu_id = 13 }, { perf_thread_cycles = 1697757, perf_thread_cpu_migrations = 0 }, { cookie = 0 }
  [12:05:08.307655201] (+2.391579976) laura sip_core:scheduled_executor_task_begin: { cpu_id = 14 }, { perf_thread_cycles = 8044, perf_thread_cpu_migrations = 0 }, { executor = "manager", source_filename = "upnp/protocol/pupnp/pupnp.cpp", source_line = 397, cookie = 1 }
  [12:05:08.307760459] (+0.000105258) laura sip_core:scheduled_executor_task_end: { cpu_id = 14 }, { perf_thread_cycles = 137657, perf_thread_cpu_migrations = 0 }, { cookie = 1 }
  [12:05:25.916502877] (+17.608742418) laura sip_core:scheduled_executor_task_begin: { cpu_id = 13 }, { perf_thread_cycles = 1858657, perf_thread_cpu_migrations = 0 }, { executor = "natpmp", source_filename = "upnp/protocol/natpmp/nat_pmp.cpp", source_line = 233, cookie = 2 }
  [12:05:25.918499760] (+0.001996883) laura sip_core:scheduled_executor_task_end: { cpu_id = 13 }, { perf_thread_cycles = 2877865, perf_thread_cpu_migrations = 0 }, { cookie = 2 }
...
identifier of the process executing sip_core at the time of the trace.  You should
...
might be interested to see the defered execution of sip_core's executors.  For this,
