...
enter an interactive prompt that allows you to interact with sip_core using the
...
Guile needs primitive bindings to communicate with sip_core.  Usually, these
...
reasons:
  ...
  1. sip_core source code is in C++ not C.
  2. Dynamic loading is not present on some platform supported by sip_core.
...
of bindings that can run on any platform where sip_core is supported, bindings
...
``test/agent/src/bindings/account.h`` has all the sip_core's bindings for managing
...
Modules such as ``(agent)`` and ``(sip_core logger)`` can be compiled using the
...
While developing scenarios, you don't want sip_core's output to clobber Guile's
output.  One easy way to do so is by using the ``SIP_CORE_LOG_FILE`` environment
variable.

For example::

  SIP_CORE_LOG_FILE=sip_core.log ./agent.exe -s my-scenario.scm

But sometime you want sip_core's output on the console.  The agent disable the
console output for sip_core, but you can do the following as a workaround::

  SIP_CORE_LOG_FILE=/dev/stdout ./agent.exe -s my-scenario.scm
...
program will keep getting interrupted.  Here's a recommended GDB file to use::
     ...
     set environment XDG_CONFIG_HOME=/tmp/sip_core-agent
     set environment XDG_CACHE_HOME=/tmp/sip_core-agent
     set environment XDG_DATA_HOME=/tmp/sip_core-agent
     ...
     set environment SIP_CORE_LOG_FILE sip_core.log
