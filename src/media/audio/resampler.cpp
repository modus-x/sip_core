...
Resampler::reinit(const AVFrame* in, const AVFrame* out)
{
     ...
     */
    AVChannelLayout ch_layout_5_1;
    av_channel_layout_from_mask(&ch_layout_5_1, AV_CH_LAYOUT_5POINT1);

    AVChannelLayout ch_layout_5_1_back;
    av_channel_layout_from_mask(&ch_layout_5_1_back, AV_CH_LAYOUT_5POINT1_BACK);

    if (av_channel_layout_compare(&in->ch_layout, &ch_layout_5_1) == 0
        || av_channel_layout_compare(&in->ch_layout, &ch_layout_5_1_back) == 0) {
...
Resampler::resample(const AVFrame* input, AVFrame* output)
{
    ...
    if (ret & AVERROR_INPUT_CHANGED || ret & AVERROR_OUTPUT_CHANGED) {
        ...
        if (initCount_ > 1) {
            std::string msg = "Infinite loop detected in audio resampler";
            throw std::runtime_error(msg);
        }
        reinit(input, output);
...
Resampler::resample(const AudioBuffer& dataIn, AudioBuffer& dataOut)
{
    ...
    output->ch_layout.nb_channels = dataOut.channels();
