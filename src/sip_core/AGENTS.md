...
| `callmanager_interface.h`           | Call control: `placeCall`, `placeCallWithMedia`, `accept`, `acceptWithMedia`, `refuse`, `hangUp`, `hold`, `unhold`, `transfer`, `attendedTransfer`, `muteLocalMedia`, `muteRemoteMedia`, `requestMediaChange`, `answerMediaChangeRequest`, `getCallDetails`, `getCallList`. Conference verbs: `joinParticipant`, `createConfFromParticipantList`, `addParticipant`, `addMainParticipant`, `detachLocalParticipant`, `detachParticipant`, `joinConference`, `hangUpConference`, `holdConference`, `unholdConference`, `getConferenceList`, `getParticipantList`, `moveParticipant`, `getConferenceDetails`, `setConferenceLayout`, `muteParticipant`, `muteStream`, `hangupParticipant`, `setModerator`, `setActiveStream`, `raiseHand`, `raiseParticipantHand`. Also: `playDTMF`, `sendTextMessage`, `toggleRecording`, `startSmartInfo`/`stopSmartInfo`, `startRecordedFilePlayback`/`stopRecordedFilePlayback`. Defines `CallSignal` struct (StateChange, IncomingCallWithMedia, MediaNegotiationStatus, ConferenceCreated, RecordingStateChanged, TransferStateChange, IncomingMessage, …). |
| `configurationmanager_interface.h`  | Accounts (`addAccount`, `removeAccount`, `setAccountDetails`, `getVolatileAccountDetails`, `sendRegister`), codecs (`getActiveCodecList`, `setActiveCodecList`), audio devices (`setAudioInputDevice`, `setAudioOutputDevice`, `getAudioInputDeviceList`, `getAudioOutputDeviceList`, `setRingtone`, `setAudioManager`), audio processing (`setNoiseSuppressState`, `setEchoCancellerState`, `setAgcState`, `setVADState`, `setWebRtcParams`), messaging (`sendAccountTextMessage`, `cancelMessage`, `getMessageStatus`, `setMessageDisplayed`, `getLastMessages`), transport (`switchTransport`), and push tokens. Defines `WebRtcParams`, `TransportType`, `Message` struct, `AudioSignal`, `ConfigurationSignal`. |
| `videomanager_interface.h`          | Video devices (`getDeviceList`, `getCapabilities`, `getSettings`, `applySettings`, `setDefaultDevice`, `getDefaultDevice`, `setDeviceOrientation`, `getDeviceParams`), audio device start/stop (`startAudioDevice`/`stopAudioDevice`), input management (`openVideoInput`/`closeVideoInput`, `createMediaPlayer`/`closeMediaPlayer`), media player controls (`pausePlayer`, `mutePlayerAudio`, `playerSeekToTime`), sinks (`registerSinkTarget`, `startShmSink`, `getRenderer`), recorder (`startLocalMediaRecorder`, `stopLocalRecorder`), dynamic device registration (`addVideoDevice`/`removeVideoDevice`), raw frame injection (`getNewFrame`/`publishFrame`). Defines `MediaFrame` / `AudioFrame` / `VideoFrame` wrappers around `AVFrame`/`AVPacket` + `SinkTarget` + `VideoSignal` / `MediaPlayerSignal`. Heavy `#ifdef` for Android/iOS — JNI glue (`acquireNativeWindow`, `captureVideoFrame`, `setVideoFrame`, etc.) is here. |
