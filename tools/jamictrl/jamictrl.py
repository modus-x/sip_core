...
from tester import libsip_coreTester
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test', help=' '.join(str(test) for test in libsip_coreTester().getTestName() ), metavar='<testName>')
    parser.add_argument('--auto-answer', help='Keep running and auto-answer the calls', action='store_true')

    args = parser.parse_args()

    ctrl = libsip_coreCtrl(sys.argv[0], args.auto_answer)
    ...
    if args.test:
        libsip_coreTester().start(ctrl, args.test)
