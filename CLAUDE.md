...
3. **Stack — `src/sip/` + `src/media/`.** `src/sip/` is the PJSIP integration: `SIPVoIPLink` is the PJSIP bridge, `SIPAccount`/`SIPAccountBase` handle registration and credentials, `SIPCall` drives call state and media negotiation, `SDP` parses/generates session descriptions, `SIPTransport` covers UDP/TCP/TLS. `src/media/` has per-platform audio backends (`coreaudio/`, `pulseaudio/`, `wasapi/`, `opensl/`) plus `audio-processing/` (WebRTC NS/AEC/AGC), and per-platform video backends (`osxvideo/`, `iosvideo/`, `v4l2/`, `winvideo/`, `androidvideo/`) with FFmpeg encode/decode. `src/client/`, `src/connectivity/`, `src/im/`, `src/compat/` are support modules.
