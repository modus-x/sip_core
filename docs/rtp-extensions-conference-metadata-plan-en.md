...
VideoMixer::process()  builds SourceInfo{x,y,w,h,hasVideo,callId,streamId}   src/media/video/video_mixer.cpp (layout: updateLayout() :427)
  -> onSourcesUpdated_ callback (registered Conference::attachVideoMixerCallbacks)   src/conference.cpp:123
      -> ParticipantInfo{...} built, canvas w/h from mixer   src/conference.cpp:172-262
      -> Conference::updateConferenceInfo(newInfo)   src/conference.cpp:293 / body :2143
          -> Conference::sendConferenceInfos()   src/conference.cpp:989   [#if CONFERENCE_METADATA ~:991]
              -> foreachCall: call->sendConfInfo(getConfInfoHostUri(...).toString())   src/conference.cpp:993-1001
...
Voice activity (throttled, separate): Conference::sendVoiceActivity() :1017 -> flushVoiceActivity() :1048 -> doSendVoiceActivity() :1059
Layout change: Conference::setLayout() :857 -> sendConferenceInfos()
```
Wire JSON: `ConfInfo::toString()` `src/conference.cpp:975` = `{"p":[...],"w","h","v":1,"layout"}`; per-participant `ParticipantInfo::toJson()` `src/conference.h:99-118`. Voice: `voiceActivivtyToString()` `src/conference.cpp:2108` = `[{uri,sinkId,state}]`.
...
Call::onTextMessage   src/call.cpp:355
  ...
  "application/confOrder+json"        -> Conference::onConfOrder(...)           src/conference.cpp:1483  (HOST only; KEEP on SIP)
...
- **`0x04` layoutVersion (fallback only)** — `data` = `[gen:2]` (the layout generation). Used **only** in the coords-on-SIP fallback (§9) where `0x02` is not emitted; in the primary path `gen` rides inside `0x02` and `0x04` is absent.

> **Two independent id namespaces (don't conflate):** the SDP `extmap` ids (`1`/`2`/`4`, §4) identify *which URI* the header-extension carries and live in the RTP fixed-header extension map; the in-block **element ids** (`0x01`/`0x02`/`0x03`/`0x04`, above) identify *which event* inside our `urn:svetets:participant-info` payload. They are separate spaces — the visual coincidence of extmap id `4` (voice-activity) and element id `0x04` (layoutVersion) is not a collision.
...
**Latch `gen` the same way (Kirill Q3/Q4).** The layout generation stamped into `0x02` (and, in the §9 fallback, the `0x04` version element) MUST use this **same pts-keyed, sticky-per-access-unit latch** — store `pts → {captureNtp, gen}` together. Read `gen` from the `Conference` layout counter (under `confInfoMutex_`) **at the pre-encode site**, not live in `writeCallback`: the mixer's `onSourcesUpdated_` fires synchronously *before* `publishFrame`, so the frame's layout is known at encode time, but a layout change that lands mid-fragmentation must **not** retro-stamp already-flushed packets. Latching `gen` per access unit guarantees every packet of frame N carries frame N's generation — the exact property the receiver's version-gating (§6.3) relies on.
...
**Mandated carrier = AVFrame side-data (M4).** Attach the RTP timestamp + layout `gen` (and the parsed geometry, or a key to it) as **`AVFrame` side-data** in the receive path *before* decode, so it travels **with** the frame through the decoder to `SinkClient`. **Precedent in-tree:** the receive path already attaches side-data — `av_frame_new_side_data_from_buf(frame, AV_FRAME_DATA_DISPLAYMATRIX, …)` at `video_receive_thread.cpp:105-106` — so a custom `gen`/geometry side-data type follows an established pattern. **Reject** the "ordered per-`SocketPair` queue applied in decode order" alternative: the decoder **drops late frames** (`media_decoder.cpp:842`), so arrival order desyncs from decoded-frame order and the queue would misalign metadata to frames. Side-data is the only carrier that survives frame drops.
...
**Receive rule when a frame's `gen` snapshot is not yet complete (version-gating fallback — Kirill Q3).** A frame declares its `gen`; the client must cut it with **that** `gen`'s coordinate table, never an older one. When the table for a multi-fragment `gen` has not fully arrived (or, in the coords-on-SIP fallback §9, the SIP roster for `gen` is late), apply this bounded ladder — **never a wrong-version cut, never a permanent freeze**:
1. **Hold** the frame ≤ 1–2 frame-times (≈33–66 ms) waiting for the table. The hold **must stay under the 200 ms drop-to-live guard** (`media_decoder.cpp:842`) or the frame is dropped as stale; and it must **not block the decoder thread** (`Observable::notify` runs synchronously under `mutex_`, `observer.h:126`).
2. **Freeze-last-good** on timeout: re-display that tile's previous correctly-cropped frame (`lastFrame_`/`obtainLastFrame`, `video_base.cpp:68-73`). A 1–2-frame tile freeze is imperceptible; a mis-crop is the visible defect. Do **not** letterbox/black a live tile.
3. **Overflow floor:** if the table never arrives and the hold buffer would overflow, fall back to **best-known table** (today's brief artifact) — a bounded glitch is preferable to an unbounded freeze.

This converts Kirill's "one new frame slips through with the old layout" into a sub-perceptual 1–2-frame hold in the good case and a bounded freeze in the bad case. **`gen` comparison uses RFC 1982 serial arithmetic**, and a **superseded `gen` releases any held frames** to the newest `gen` (or drops them) so a lost intermediate generation cannot wedge a permanent freeze.
...
**De-dup / reorder — the generation/fragment fields (wire format §3.5).** The `0x02` snapshot carries `gen` + `fragIdx`/`fragCount`; `0x03` carries `vaSeq`. Receiver: reject any `gen`/`vaSeq` ≤ last-applied (de-dup + reorder rejection, **RFC 1982 serial arithmetic** for wrap); apply a `gen` only when all `fragCount` fragments arrive (§3.0). For **fragCount==1** (the whole snapshot fits one fragment — ≤11 tiles at the default 128-byte cap per the §3.0 formula) any one surviving copy fully re-syncs; for multi-fragment `gen`s, ×3 duplication covers single-fragment loss and a newer `gen` supersedes an incomplete older one (self-heal on next change). `0x01` needs no de-dup (per-packet).

**`gen` is the cross-channel common ID (Kirill Q4).** The same monotonic generation stamps the RTP geometry **and** the SIP roster/decoration, so labels and geometry can never describe different layouts. Data model: add a fresh `uint32_t layoutSeq` to `ConfInfo` (`conference.h:162-192`) — **not** `ConfInfo::v` (`:166`, protocol version, const 1), **not** `layout` (`:167`, mode 0–2), **not** `VideoMixer::layoutUpdated_` (a consumable pending-count); bump it at the single geometry choke point `Conference::setLayout` (`conference.cpp:857`) and every add/remove/move rebuild; serialize its low 16 bits as `"ls"` in `ConfInfo::toString` (`conference.cpp:975`). **Invariant:** the value written to the RTP extension and to the SIP JSON MUST be read from the same counter in the same critical section (`confInfoMutex_`), and the RTP `gen` MUST be latched at the pre-encode AVFrame site (reuse the `0x01` NTP latch, §5.3) — never read live in `writeCallback`, or a layout change mid-fragmentation stamps gen-N pixels with gen-N+1 (the very wrong-version cut we prevent).

**Delivery control — pull + heartbeat (Kirill Q2).** On-change ×3 is not a delivery guarantee (a multi-second stall loses all three copies). Because the per-frame `gen` tells the receiver it is behind, add a **pull**: when a frame declares a `gen` the client has no snapshot for, it sends `requestLayout(gen)` on the `confOrder` bus (model on the ungated scalar verb `shareState`, `conference_protocol.cpp:122-129/212-219`; precedent `SIPCall::sendKeyframe` `sipcall.cpp:2516` = RTCP-PLI "re-emit current state"); the host replies by re-running `sendConferenceInfos()`. Also add a **low-rate host heartbeat** re-emit of the current snapshot (today every `sendConferenceInfos()` call site is on-change — there is no heartbeat) to self-heal a single lost update without any client action. Both are cheap and bounded, and both depend on the `gen` tag existing.
...
**Considered and rejected as primary — "coordinates on SIP, only a version tag on RTP" (Kirill Q1 minimize).** Tempting for MTU minimization: RTP carries only `0x01` NTP + a 2-byte `0x04 layoutVersion` (`gen`), and the full coordinate table rides SIP tagged with `layoutSeq`. **Honest concession:** on *steady-state bandwidth* this variant actually wins for large, layout-stable conferences (no permanent 128–256 B per-packet reservation, no `0x02` fragmentation, simpler receive) — so the rejection is **not** on bandwidth grounds. It is rejected as the primary mechanism because of its interaction with the version-gating we require: (1) **skew becomes a freeze** — it does not remove the SIP-minus-RTP latency; combined with version-gating it converts a sub-perceptual wrong-cut into a per-layout-change freeze whose length equals that skew (unbounded tail). With geometry on RTP the skew is zero. (2) **join regression** — a joining tile has a version stamp but **no geometry** until SIP lands → black/frozen tile, vs the primary path's render-immediately (§7 join-race). *(Note: the SIP flood-protection at `conference.cpp:1019-1021` is about voice-activity flipping 16+/s; layout/coordinate changes are join/leave/move-frequency and already ride SIP `confInfo` today unthrottled — so flood is NOT a reason against coords-on-SIP per se; it only becomes relevant if the delivery-control heartbeat/`requestLayout` re-sends are pushed aggressively over SIP.)* **Kept as a negotiated fallback** for servers/peers that cannot emit the `0x02` geometry extension: `0x02` is dropped, the `0x04 layoutVersion` (2 B `gen`) element carries the tag on RTP, coordinates ride SIP tagged with `layoutSeq`, and the §6.3 hold/freeze/floor + §7 heartbeat/`requestLayout` mitigations become **mandatory**. This is a degraded interop path, not the design.
...
| 13 | `src/conference.cpp` | `sendConferenceInfos` (989), `doSendVoiceActivity` (1059) | Route geometry/VA/NTP to RTP; keep hybrid decoration + `confOrder` on SIP when extmap negotiated |
...
5a. **Sender-side delivery-control obligations (Kirill Q2).** If a dedicated external MCU is the mixer (not sip_core, §0), the Q2 delivery guarantees are **server obligations, not client-only code**: the mixer MUST (i) answer `requestLayout(gen)` by re-emitting the current snapshot, (ii) send a periodic snapshot **heartbeat**, and (iii) know its own current `gen`. If it will not, delivery-control degrades to the client-side hold/freeze/floor only (§6.3) — the frame is never mis-cut, but a lost update is only recovered on the *next* layout change. Confirm the MCU will implement (i)/(ii).
...
- Send (SIP, in-dialog INFO): `conference.cpp:989/1017/1048/1059/857/2108/2075`, `call.cpp:784/769/799`, `sipcall.cpp:2233/2266/1044`, `im/instant_messaging.cpp:147/156-157`, `conference.h:99/80/120`.
- Receive (SIP): `sipvoiplink.cpp:1551/1561`, `sipaccountbase.cpp:176`, `call.cpp:355/628/689`, `conference.cpp:1483`.
